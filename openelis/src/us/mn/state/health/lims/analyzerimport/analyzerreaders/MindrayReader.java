...
public class MindrayReader extends AnalyzerLineInserter {
    ...
    private void addAnalyzerResultFromLine(List<AnalyzerResults> results, List<String> lines) {
        ...
        for (int i = 4; i < header.length; i++) {
            ...
            if (!GenericValidator.isBlankOrNull(header[i])) {
                MappedTestName mappedName = AnalyzerTestNameCache.instance().getMappedTest(AnalyzerType.Mindray_3452, testName);

                if( mappedName == null){
                    mappedName = AnalyzerTestNameCache.instance().getEmptyMappedTestName(AnalyzerType.Mindray_3452, testName);
                }
