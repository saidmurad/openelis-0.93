...
public class CobasReader extends AnalyzerLineInserter {
	...
	private static final int GGT = index++;
	...
	private void addAnalyzerResultFromLine(List<AnalyzerResults> results, List<String> lines) {
		...
		for (int i = 4; i < header.length; i++) {
			...
			if (!GenericValidator.isBlankOrNull(header[i])) {
				MappedTestName mappedName = AnalyzerTestNameCache.instance().getMappedTest(AnalyzerType.COBAS_INTEGRA400, testName);

				if( mappedName == null){
					mappedName = AnalyzerTestNameCache.instance().getEmptyMappedTestName(AnalyzerType.COBAS_INTEGRA400, testName);
				}
